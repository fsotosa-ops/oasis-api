...
@router.get(
    ...
    description="Get detailed user information including memberships. Platform Admin only.",
)
...
async def list_org_members(
    ctx: dict = Depends(OrgRoleChecker(["owner", "admin", "facilitador"])),
...
@router.delete(
    ...
    description="Remove a user from the current organization. Does not delete their account.",
)
async def remove_org_member(
    ...
    if not can_manage_role(requester_role, target_membership["role"]):
        raise HTTPException(
            status_code=403,
            detail=f"Cannot remove a {target_membership['role']}. Insufficient permissions.",
        )
